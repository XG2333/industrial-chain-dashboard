...
部署包根目录下运行：
    python process_all.py [--provider mock|deepseek] [--ai]
...
流程（对 锂电/锡/硅 各行业）：
  ...
  4. 产物复制到 dashboard data/ 并清理缓存

原始文件约定（放入 input/ 或 --input-dir 指定）：
  碳酸锂数据库.xlsx   → 锂电
  锡产业链数据.xlsx    → 锡
  硅产业链数据.xlsx    → 硅
"""
...
# 部署场景：脚本位于包根（产业看板系统/process_all.py），多skill联动 为同级子目录；
# 源仓库场景：脚本位于 多skill联动/scripts/ 下。自检测二者。
_SCRIPT_DIR = Path(__file__).resolve().parent
_DEPLOY_SKILL_DIR = _SCRIPT_DIR / "多skill联动"
PROJECT_ROOT = _DEPLOY_SKILL_DIR if _DEPLOY_SKILL_DIR.is_dir() else _SCRIPT_DIR.parent
DASHBOARD_DIR = PROJECT_ROOT.parent / "本地可视化dashboard"
SELECTING_SKILL_DIR = PROJECT_ROOT.parent / "Selecting skill"

# 源项目旧绝对路径前缀（yaml 内硬编码，运行时替换为包内路径）。
# 注意 yaml 内路径是正斜杠（C:/Users/11/...），反斜杠键只兜底其他来源
# （GBK 乱码键"澶歴kill鑱斿姩"同样为兜底，UTF-8 读取下不会命中）。
OLD_PREFIXES = {
    r"C:/Users/11/Documents/Selecting skill": str(SELECTING_SKILL_DIR).replace("\\", "/"),
    r"C:/Users/11/Documents/澶歴kill鑱斿姩": str(PROJECT_ROOT).replace("\\", "/"),
    r"C:/Users/11/Documents/多skill联动": str(PROJECT_ROOT).replace("\\", "/"),
    r"C:\Users\11\Documents\Selecting skill": str(SELECTING_SKILL_DIR),
    r"C:\Users\11\Documents\多skill联动": str(PROJECT_ROOT),
}
...
def build_workflow_yaml(industry: str, provider: str = "mock") -> Path:
    ...
    wf["run_root"] = str(PROJECT_ROOT / "workflow_runs")
    wf["final_output"] = str(PROJECT_ROOT / "output" / "<input_stem>_workflow.xlsx")
    for step in wf.get("steps", []):
        if step.get("id") == "generate_stock_targets" and "params" in step:
            step["params"]["provider"] = provider
    # steps 内嵌路径：先替换旧 python 解释器（旧 Selecting skill .venv），再替换路径前缀。
    # 顺序不能反：前缀替换会先把 Selecting skill 改为包内路径，导致旧 python 路径失配无法替换。
    replaced = rewrite(wf, {
        r"C:/Users/11/Documents/Selecting skill/.venv/Scripts/python.exe": sys.executable.replace("\\", "/"),
        r"C:/Users/11/Documents/澶歴kill鑱斿姩": str(PROJECT_ROOT).replace("\\", "/"),
    })
    replaced = rewrite(replaced, OLD_PREFIXES)
...
def process_industry(industry: str, raw_path: Path, final_name: str, provider: str, use_ai: bool) -> None:
    out = DASHBOARD_DIR / "data" / final_name
    ...
    if industry == "lithium":
        ...
        # recompute_secondary_tin_silicon 固定处理 dashboard/data/ 下锡/硅文件
        run([sys.executable, str(RECOMPUTE_TIN_SILICON)], f"2. {industry} 二次筛选列（通用规则）")

    # 个股标的
    stock_out = PROJECT_ROOT / "output" / STOCK_TARGET_FILES[industry]
    run([
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--input-dir", default=None, help="原始文件目录（默认 多skill联动/input）")
    ...
    input_dir = Path(args.input_dir) if args.input_dir else PROJECT_ROOT / "input"
    ...
    cache_dir = DASHBOARD_DIR / "data" / "cache"
    if cache_dir.exists():
    ...
    print("\n========== 全部完成 ==========")
    print(f"看板数据已就位: {DASHBOARD_DIR / 'data'}")
    print(f"个股标的已就位: {PROJECT_ROOT / 'output'}")
    print("下一步：")
    print(f"  1. cd {DASHBOARD_DIR / 'frontend'} && npm install && npm run build")
    print(f"  2. cd {DASHBOARD_DIR} && Copy-Item .env.example .env（配置 STOCK_TARGETS_DIR）")
    print(f"  3. start.bat 启动看板")
    return 0
