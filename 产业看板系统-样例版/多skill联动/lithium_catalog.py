# -*- coding: utf-8 -*-
"""Skill1 catalog generator for the lithium battery industry.

The deterministic sector/category rules live in lithium_rules.py; this module
only owns Excel scanning, catalog layout, and pickle creation.
"""
...
from lithium_rules import (
    FREQ_EN,
    classify_category,
    classify_indicator_sector,
    classify_sector,
    detect_freq,
...
def build_catalog(wp, catalog=None):
    ...
    for freq_label in ["日度", "周度", "月度", "季度", "年度"]:
        ...
        for s in sf:
            ...
            for ind in s["inds"]:
                seq += 1
                ind_sec = classify_indicator_sector(sn, ind["name"])
                vals = [seq, "", ind["freq"], ind["col"], ind["name"], ind["unit"], ind_sec]
    ...
    wb.save(wp)
    # 返回 (sheets, indicators, catalog)：catalog 供 build_tin_pickle 复用，
    # 避免重复 discover（省一遍全文件扫描）
    return len(catalog), total_inds, catalog
...
def build_tin_pickle(wp, out_path, catalog=None):
    ...
    for s in catalog:
        ...
        # 只读日期列(0)与各指标列（usecols），省去全列解析
        need_cols = sorted({0} | {ind["col"] for ind in s["inds"]})
        df = pd.read_excel(xl, sheet_name=sn, header=None, dtype=str, usecols=need_cols)
        hdr_row, unit_row = find_header_rows(df)
        ...
        # usecols 后列位置 = need_cols 中原始列号的下标
        col_pos = {orig: pos for pos, orig in enumerate(need_cols)}
        for ind in s["inds"]:
            ci = ind["col"]
            values = pd.to_numeric(df.iloc[data_start:, col_pos[ci]], errors="coerce")
