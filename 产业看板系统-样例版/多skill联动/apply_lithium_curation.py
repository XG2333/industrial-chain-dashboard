# -*- coding: utf-8 -*-
"""Post-process the lithium directory with deterministic lithium rules.

All rule tables and pure classification/tagging functions live in
lithium_rules.py; this script only owns workbook mutation and audit reporting.
"""
...
from lithium_rules import build_tags, classify_major, classify_nature, classify_sub, detect_indicator_freq
from classification_candidates import candidate_plan


SRC = Path(r"C:\Users\11\Documents\多skill联动\workflow_runs\20260811T084756Z_ff8cb7541bcc\02_financial_variable_curation\directory_marked.xlsx")
OUT = Path(r"C:\Users\11\Documents\多skill联动\output\碳酸锂数据库_workflow.xlsx")
REPORT = Path(r"C:\Users\11\Documents\多skill联动\output\碳酸锂数据库标签审计报告.md")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Apply deterministic lithium curation rules.")
    ...
    for row in ws.iter_rows(min_row=2):
        ...
        if "氯化锂" in name:
            sector = "其他锂盐"
            row[6].value = "其他锂盐"
        plan = candidate_plan(name, current_sheet, unit=unit)
        major = plan["major"]
        sub = plan["sub"]
