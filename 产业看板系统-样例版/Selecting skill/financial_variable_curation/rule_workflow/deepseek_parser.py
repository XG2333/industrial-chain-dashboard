...
class DeepSeekRuleParser:
    ...
    def __init__(
        ...
        *,
        ...
        self.settings = settings or LLMSettings.from_env(provider="deepseek")
    ...
    def parse(self, text: str, context: dict | None = None) -> RuleParserProviderResult:
        ...
        context = context or {}
        client = self.client_factory.create("deepseek")
        user_payload = {
        ...
        if not isinstance(document, RuleSetDocument):
            raise LLMStructuredOutputError("DeepSeek rule parser returned the wrong structured model.")
        document.provider = self.provider
        document.model = self.settings.deepseek_model
        document.metadata = dict(document.metadata)
        document.metadata.pop("mock_only", None)
        document.metadata["production_ready"] = False
