...
// 压缩摘要（省 token）：单位/说明只出现一次；量仓为整数、增减带符号、比与额按表格口径。
...
function buildLines(data: PositionsData): string[] {
  ...
  for (const g of data.groups ?? []) {
    ...
    for (const r of g.rows) {
      ...
      lines.push(`${tag} ${p} | 量${vol} 仓${oi} 比${ratio} 额${to}`);
    }
  }
  return lines;
}
...
function buildInstLines(data: InstData | null): string[] {
  ...
  lines.push(`# 各机构成交持仓 ${date}（交易所会员持仓排名, 收盘后公布; 量/多/空三栏独立排名）`);
  for (const g of data.groups) {
    ...
    for (const r of g.rows.slice(0, 12)) {
      ...
      lines.push(`${r.rank} 量:${r.vol_member} ${v} | 多:${r.long_member} ${l} | 空:${r.short_member} ${s}`);
    }
  }
  return lines;
}
...
export function PositionQuickAnalysis({ dataset }: { dataset: string }) {
  ...
  const run = async () => {
    ...
    // 机构数据随点随取(至多等待 ~5s; 失败仅缺第二部分, 不阻塞)
    if (!i || i.groups.length === 0) {
      const startedAt = Date.now();
      while ((!i || i.groups.length === 0) && Date.now() - startedAt < 5000) {
        i = await fetchInst();
        if (i && i.groups.length > 0) break;
        await new Promise((r) => setTimeout(r, 300));
      }
    }
      ...
      <div className="px-4 py-3">
        {!analysis && !loading && !error && (
          <p className="py-1 text-xs text-muted-foreground">
            基于上方各合约量仓数据与各机构会员持仓排名（多空头部席位与增减）生成简洁点评
          </p>
        )}
