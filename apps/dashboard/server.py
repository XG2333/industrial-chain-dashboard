...
DATA_DIR = Path(os.getenv("DASHBOARD_DATA_DIR") or (REPO_ROOT / "data" / "processed")).resolve()
CACHE_DIR = Path(os.getenv("DASHBOARD_CACHE_DIR") or (REPO_ROOT / "runtime" / "cache")).resolve()
CACHE_VERSION = 6
TIN_WORKBOOK_PATH = DATA_DIR / "锡产业链数据_workflow_ai.xlsx"
SILICON_WORKBOOK_PATH = DATA_DIR / "硅产业链数据_workflow_ai.xlsx"
LITHIUM_WORKBOOK_PATH = DATA_DIR / "碳酸锂数据库_workflow_ai.xlsx"
STOCK_TARGETS_DIR = Path(os.getenv("STOCK_TARGETS_DIR") or (REPO_ROOT / "data" / "stock-targets")).resolve()
STOCK_TARGET_FILES = {
    "silicon": "个股标的_硅.xlsx",
    "tin": "个股标的_锡.xlsx",
...
app.add_middleware(CORSMiddleware,
    allow_origins=["http://localhost:3000", "http://127.0.0.1:3000",
                   ...
                   # 离线快照(单文件 HTML, file:// 直开)以 Origin: null 访问本地接口
                   "null"],
...
def _ensure_gfex_history(dataset: str) -> dict | None:
    ...
    # 数据结构: wh={仓库:{product,vals}}, sub={品种:{vals}}, tot=[...]; vals 与 dates 同序(最新在前)
    # tot 口径迁移: gfex 接口单日返回"全品种合页"(含铂/钯等无关品种), 旧版把全品种"总计"行存进 tot,
    # 导致 全国仓单量(锂电页)把铂钯/其他品种也算进来。sub 只收本 dataset 关注品种 → 由 sub 重建 tot。
    if hist.get("tot_src") != "varieties" and (hist.get("dates") or []):
        n = len(hist["dates"])
        rebuilt = []
        for i in range(n):
            vals = [r["vals"][i] for r in hist["sub"].values()
                    if len(r["vals"]) > i and r["vals"][i] is not None]
            rebuilt.append(round(sum(vals), 2) if vals else None)
        hist["tot"] = [v for v in rebuilt if v is not None]
        hist["tot_src"] = "varieties"
    base = time.strftime("%Y-%m-%d")
    lt = time.localtime()
    released_today = not (lt.tm_hour < 15 or (lt.tm_hour == 15 and lt.tm_min < 30))
    # 目标窗口 = 以"最新应公布日"(今日 15:30 后 / 上一交易日)起向前 _WR_HIST_DAYS 个交易日
    cands: list[str] = []   # 近→远, 跳过周末去重
    seen: set[str] = set()
    back0 = 0 if released_today else 1
    for back in range(back0, 70):
        d8 = _prev_trade_day8(base, back)
        if d8 in seen:
            continue
        seen.add(d8)
        cands.append(d8)
        if len(cands) >= _WR_HIST_DAYS:
            break
    cand_set = set(cands)
    # 1) 丢弃窗口外的旧日期(dates/各序列同步截断: 旧日期位于列表尾部)
    if any(d not in cand_set for d in hist["dates"]):
        hist["dates"] = [d for d in hist["dates"] if d in cand_set]
        n = len(hist["dates"])
        ...
        if len(hist["tot"]) > n:
            hist["tot"] = hist["tot"][:n]
    have = set(hist["dates"])
    # 2) 缺失候选(均为比 have 更近的缺口, 旧→新排序) — 逐日补抓, 每次插入头部保持 最新在前
    need = [d for d in cands if d not in have]           # 近→远
    need.sort()                                          # 旧→新(字符串日期同语义)
    for d8 in need:
        rows = _gfex_fetch_day(d8)
        if not rows:
            continue  # 非交易日/接口无数据: 当日不占位, 下次再补
        ...
        # tot = 当日关注品种小计之和(不再取 gfex 全品种"总计"行——该行混入铂/钯等无关品种)
        sub_vals = [s["now"] for s in sub.values() if s.get("now") is not None]
        if sub_vals:
            hist["tot"].insert(0, round(sum(sub_vals), 2))
    # 3) 修剪回窗口上限
    if len(hist["dates"]) > _WR_HIST_DAYS:
        keep = _WR_HIST_DAYS
        hist["dates"] = hist["dates"][:keep]
        for rec in list(hist["wh"].values()) + list(hist["sub"].values()):
            if len(rec["vals"]) > keep:
                rec["vals"] = rec["vals"][:keep]
        if len(hist["tot"]) > keep:
...
def _gfex_warehouse_live(dataset: str) -> dict:
    ...
    want = _WR_GFEX_VARIETIES.get(dataset) or set()
    multi = len(want) > 1
    rows_out: list[dict] = []
    ...
    # 品种小计行: 仅多品种(硅: 多晶硅+工业硅)时保留; 单品种(锂)时全国行即该品种合计, 避免重复
    if multi:
...
def _fetch_sina_future_quotes(codes: list[str]) -> dict[str, dict]:
    ...
    [8]最新价 [10]昨结 [13]持仓量 [14]当日成交量 [17]行情日期。
...
def _sina_contract_quotes(codes: list[str]) -> dict[str, dict]:
    ...
    新浪 nf_ 月合约字段: [13]=持仓量 [14]=当日累计成交量。
    已与新浪日K(getDailyKLine: v=成交量 p=持仓量)逐合约核对:
    两个来源的持仓量日间只差 ~±2%(换月日除外), 成交量则日间大幅波动 → 13/14 与 v/p 一一对应。
    """
    out: dict[str, dict] = {}
    for i in range(0, len(codes), 40):
        ...
        for line in raw.strip().splitlines():
            ...
            try:
                price = float(vals[8])
                oi = float(vals[13])   # 持仓量
                vol = float(vals[14])  # 当日成交量(累计)
            except (TypeError, ValueError):
                continue
            if vol < 0 or oi < 0:
                continue  # 未开盘时段字段为 0/- 占位
            if price <= 0:
                continue
...
def _load_pos_snapshot() -> dict:
    """读 量/仓 收盘快照；兼容旧格式 {date, rows} → 迁移为 {history: {date: rows}}。"""
    try:
        if _POS_SNAPSHOT_FILE.exists():
            raw = json.loads(_POS_SNAPSHOT_FILE.read_text(encoding="utf-8"))
            if "history" in raw:
                return raw
            if raw.get("date") and raw.get("rows"):
                return {"history": {raw["date"]: raw["rows"]}}
    except Exception:
        pass
    return {"history": {}}
...
def _maybe_save_pos_snapshot(groups: list[dict], date: str):
    """交易日 15:30 后把当日 量/仓 存入历史(保留近 6 个交易日, 供计算「较前日」)。

    只在「本地日期 == 行情日期」时落盘: 夜盘时段(21:00 后行情日期已滚动到下一
    交易日、当日数据未走完)与节假日都不存, 避免存下不完整的一天覆盖整日数据。
    """
    try:
        lt = time.localtime()
        ...
        hist[date] = rows
        # 只留最近 6 个交易日(逐日增减只需前一日, 多留几天容错节假日/漏存)
        while len(hist) > 6:
            hist.pop(min(hist))
...
def _fetch_positions_live(dataset: str) -> dict | None:
    ...
    # 「较前日」基线 = 距 base 最近且严格早于 base 的已收盘快照。
    # 用严格小于号: 若 15:30 后当日快照已落盘, 实时数据与当日快照同源, 相减恒为 0 →
    # 必须退回前一日快照, 快照日期等于当日(=无前日基线)时显示 —(null) 而不是假 0。
    hist = _load_pos_snapshot().get("history", {})
    prev_date = max((d for d in hist if d < base), default=None)
    snap_rows = hist.get(prev_date, {}) if prev_date else {}
    groups: list[dict] = []
    quote_time = ""
...
仓单数据为分层结构（单位：手；日/周/月环比 = 最新 vs 前1/5/21个交易日的手数差，如 "日+420 周+1590 月+449 手"，与仓单日报表列数值同口径，不是百分比）。**一律以输入数据为准：输入出现哪些行才分析哪些，未出现的指标（如成交量/持仓量/成交持仓比/量价行——公开 API 快照通常只含仓单量）绝不提及或臆测。**
数据形态可能为以下两种之一：
...
要求：
0. 逻辑完整：每条点评按 现象→原因→影响/含义 的链条表述，先给判断再给依据再讲含义，禁止只罗列数据或堆砌指标；用词规范：成交量/持仓量/成交持仓比一律写全称（禁止缩写成"量/仓/量仓/比"），提及仓单一律写"仓单量"；
...
{{"verdict": "一句话判断仓单整体状态与主导逻辑（25-40字）",
  "points": [{{"metric": "仓库/地区/量价名", "change": "最新值与环比手数差（如：10627手，日+420 周+1590 月+449 手）", "comment": "一句话点评（≤30字）"}}],
  "outlook": "一句话前瞻判断（≤35字）"}}"""
...
【品种】为该品种小节，其后行为合约行，字段：合约 最新价 | 成交量(手, 较前一日增减) 持仓量(手, 较前一日增减) 成交持仓比% 成交额。
合约以到期月代码表示（如 2701），持仓量最大者为主力，标注为 主力(2701)。

第二部分·各机构成交持仓（若提供；交易所会员持仓排名, 数据截至标题日期）：
每行三栏独立排名（同行机构可能不同），字段：名次 | 成交量:当日成交量第N名会员 成交量(较前一日) | 多头:多头持仓第N名会员 多头持仓量(较前一日) | 空头:空头持仓第N名会员 空头持仓量(较前一日)。

{text}

要求：
0. 逻辑完整：每条点评按 现象→原因→影响/含义 的链条表述，先给判断再给依据再讲含义，禁止只罗列数据或堆砌指标；用词规范：成交量/持仓量/成交持仓比一律写全称，禁止缩写成"量/仓/量仓/比"；
1. 先点评整体参与度：主力合约的成交量/持仓量/成交持仓比及较前一日增减，判断资金进场还是离场；
2. 再点评月份结构：持仓量向哪些月份集中、近远月活跃差异，点名 2-4 个变化最大的合约并用数字支撑；
3. 结合成交量与持仓量的同向/背离（成交量与持仓量同步放大=新资金进场、成交放大而持仓量下降=短线资金离场、成交萎缩而持仓量上升=锁仓观望等）给出价格含义前瞻；
...
{{"verdict": "一句话判断市场参与度与资金行为的主导逻辑（25-40字）",
  "points": [{{"metric": "合约/成交量持仓量/机构", "change": "最新值与较前一日增减（如：175248手 +9421；中信期货多30191 +53）", "comment": "一句话点评（≤30字）"}}],
  "outlook": "一句话前瞻判断（≤35字）"}}"""
    elif payload.kind == "market_overview":
        # 主力期货总体分析(kind=market_overview): 盘面+技术+相关股票/资金+仓单+成交持仓
        prompt = f"""你是{industry_name}研究专家，请对「{payload.sector}」做一份深入、多维度的总体分析。

输入分多节(数字即口径, 以输入为准, 绝不虚构未提供维度; 未提供的小节直接跳过不写):
- 实时行情: 现价/涨跌%/昨结/今开/当日高/低;
- 当日分时要点: 现价位于当日区间百分位、极值时间等(若给出);
- 日K窗口: 近5/10/20日涨跌幅、近60日区间高低与回撤、近10日收盘序列;
- 资金参与(若给出): 主力合约当日成交额、成交量与持仓量及较前一日增减、成交持仓比——期货无官方"净流入/净流出"数值, 该节用于推断资金进出倾向, 不得虚构具体净流入金额;
- 相关股票(若给出): 板块样本个股涨跌家数与平均涨跌、涨幅/跌幅居前个股(点名 2-3 只, 用其名称+数值)、
  主力净流入居前个股(名称+金额, 判断产业资金情绪);
- 仓单(若给出): 交易所仓单 全国总量/日增减与近期累计变化(仓库级增减可点名 1-2 个);
- 各合约持仓: 各合约持仓集中度/近月远月(点名 2-3 个变化大合约);
- 各机构席位(若给出): 多空头部会员与净倾向。

{text}

要求：
0. 逻辑完整：各小节按 现象→原因→影响/含义 的链条表述, 先判断后依据再含义, 不罗列数字；用词规范：成交量/持仓量/成交持仓比一律写全称, 禁止缩写成"量/仓/量仓/比"；
1. 一句话定性: 当前位置与主导逻辑;
2. 盘面与技术 1-2 条(分时强弱、窗口涨跌与回撤);
3. 资金参与 1 条(若给出): 由主力合约成交额规模与成交量/持仓量较前一日增减判断资金进出倾向(成交放量且增仓→资金流入做多、放量减仓→分歧/短线离场、缩量减仓→资金离场观望等);
4. 股票联动 1-2 条(个股涨跌结构、主力净流入方向是否与期货同向, 用个股的真实净流入金额);
5. 仓单/库存 1 条(累库还是去库、节奏, 对近月压力的含义);
6. 合约持仓与机构 1-2 条(成交量与持仓量背离、集中度、主力席位方向);
7. 前瞻 1 句; 全文 250-380 字, 有条理、不客套、不重复。

严格输出 JSON（不要 markdown 代码块、不要任何额外文字）：
{{"verdict": "一句话定性当前位置与主导逻辑（25-40字）",
  "points": [{{"metric": "维度/项目", "change": "数值（如：现价141400 -0.46%；某股主力净流入+1.2亿；仓单日+463）", "comment": "一句话点评（≤32字）"}}],
  "outlook": "一句话前瞻判断（≤40字）"}}"""
    elif payload.kind == "stock_overview":
        # 个股速评(kind=stock_overview): 自选样本整体 + 分板块明细, 结合公开产业消息给原因与短线前瞻
        news_lines: list[str] = []
        try:
            news_url = (f"https://np-weblist.eastmoney.com/comm/web/getFastNewsList?client=web&biz=web_724"
                        f"&fastColumn=102&pageSize=100&req_trace={time.time()}")
            with urllib.request.urlopen(urllib.request.Request(
                    news_url, headers={"User-Agent": "Mozilla/5.0", "Referer": "https://kuaixun.eastmoney.com/"}), timeout=12) as resp:
                nd = json.loads(resp.read()).get("data") or {}
            for it in (nd.get("fastNewsList") or [])[:20]:
                title = (it.get("title") or "").strip()
                if title:
                    news_lines.append(title[:90])
        except Exception:
            pass
        news_blob = "\n".join(news_lines) if news_lines else "(产业新闻源暂不可用, 请基于行情数据推断)"
        prompt = f"""你是{industry_name}研究专家，请对「{payload.sector}」的自选股票样本做一份深入「个股速评」（整体 + 分板块两部分）。

输入结构（以输入为准，绝不虚构输入未提供的个股/板块）：
- 样本行: 全市场样本只数与涨跌平只数、平均涨跌幅；
- 领涨/领跌: 涨幅/跌幅居前个股（名称+涨跌幅）；
- 主力净流入居前个股（名称+金额，亿）；
- 板块行: 每个细分板块一行或一组（板块名 N只 X涨/Y跌 平均涨跌%; 板块内领涨/领跌个股 名称+涨跌幅）。
change 字段格式统一为: 「N涨M跌 平均涨跌% 涨:领涨股名+涨跌幅 跌:领跌股名+涨跌幅」(领涨与领跌都必须给出, 以输入为准)。

{text}

# 近日产业公开消息（仅作分析背景, 只可引用下面列出的消息, 不得编造消息; 与个股/板块无直接关联的消息可不用）:
{news_blob}

要求：
0. 逻辑完整：每条点评按 现象→原因→影响/含义 的链条表述，先给判断再给依据再讲含义，禁止只罗列数据或堆砌指标；
1. 【整体】整体速评 80-120 字: 赚钱效应(涨跌比/平均)、资金风格、结构性主线；若有相关产业消息(如排产/涨价/招标/政策), 用 1 句说明其对样本的影响逻辑；
2. 【板块】逐板块点评(有几个板块就点评几个): 每板块输出**一行 comment, 严格按三段式**——
   「领涨:{{个股名}}—{{一句话驱动}}」;「领跌:{{个股名}}—{{一句话原因}}」;「短线:{{一句话前瞻}}」;
   领涨与领跌**都必须点名**(以输入 change 为准), 讲个股自身情况与逻辑(消息面/资金面/基本面, 有对应公开消息必须点明);
   **comment 内严禁重复任何涨跌幅/涨跌只数等数值**(数值已由 change 字段展示), 全行尽量 ≤90 字;
3. 公开消息引用要克制准确, 不客套、不重复。

严格输出 JSON（不要 markdown 代码块、不要任何额外文字）：
{{"overall": "整体速评文本（80-120字）",
  "sectors": [{{"name": "板块名", "change": "涨跌结构 + 领涨领跌股(如: 8涨11跌 +0.15% 涨:藏格矿业+2.98% 跌:江特电机-1.20%)", "comment": "三段式单行(≤90字): 领涨:{{名}}—{{驱动}};领跌:{{名}}—{{原因}};短线:{{前瞻}}; 不含涨跌数值"}}]}}"""
    else:
...
- **环比均为绝对差值（最新值 − 前一期/前月基期，单位与指标一致，如 +420 元/吨、+0.5 万吨），不是百分比**
{text}
...
{{"verdict": "一句话判断板块状态并说明主导逻辑（25-40字）",
  "points": [{{"metric": "指标名", "change": "最新值与差值数据（如：7650元/吨，月环比+380元/吨；产量环比+0.5万吨）", "comment": "一句话点评（≤30字）"}}],
  "outlook": "一句话前瞻判断（≤30字）"}}"""
    try:
        # 个股速评需要整体+全板块明细(含原因与前瞻), 输出较长, 上限放宽避免截断
        max_tokens = 2600 if payload.kind == "stock_overview" else 800
        req = urllib.request.Request(
            "https://api.deepseek.com/v1/chat/completions",
            data=json.dumps({
                "model": "deepseek-chat",
            ...
            headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"})
        raw = json.loads(urllib.request.urlopen(req, timeout=60).read())["choices"][0]["message"]["content"].strip()
        # 容忍 AI 加 ```json 围栏/前后说明文字: 逐级提取 JSON 对象
        data = None
