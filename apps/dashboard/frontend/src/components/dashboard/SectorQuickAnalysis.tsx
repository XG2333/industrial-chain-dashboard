...
// 环比差值(最新 - 前一期/前月基期, 单位与指标相同, 非百分比): 大数取整, 小数自适应保留
const diffOf = (cur: number | null, base: number | null): string => {
  if (cur == null || base == null) return "—";
  const d = cur - base;
  if (d === 0) return "0";
  const abs = Math.abs(d);
  const s =
    abs >= 100 ? String(Math.round(abs))
    : abs >= 1 ? String(Math.round(abs * 10) / 10)
    : abs.toFixed(2);
  return `${d > 0 ? "+" : "-"}${s}`;
};
...
// 单指标行（非族）：名称(单位): 最新值(日期) | 日/周环比(差值) | 月环比(差值) | 走势
function singleLine(chart: ChartMeta, data: ChartData): string | null {
  ...
  return `${name}${unit}: 最新${fmtNum(v1)}(${(pts[0].date || "").slice(5)}) | ${chgLabelOf(weekly)} ${diffOf(v1, v2)} | 月环比 ${diffOf(v1, monthlyBase(raw))} | 走势 ${trendArrows(pts)}`;
}
...
function familyLine(familyKey: string, members: ChartMeta[], dataMap: Map<string, ChartData>): string | null {
  ...
  for (const c of members) {
    ...
    parts.push(`${short} ${fmtNum(v1)}(${chgLabelOf(weekly)}${diffOf(v1, v2)})`);
