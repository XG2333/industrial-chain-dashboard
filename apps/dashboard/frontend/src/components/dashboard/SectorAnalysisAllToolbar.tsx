...
const STOCK_KEY_PREFIX = "stock_analysis_v2"; // 与 StockQuickAnalysis 缓存 key 前缀一致
...
export function SectorAnalysisAllToolbar({ groups, autoFill = false }: { groups: IndustryGroup[]; autoFill?: boolean }) {
  const [idx, setIdx] = useState<number | null>(null);
  const running = idx != null;
  const dataset = datasetOf(groups);

  // 首开自动填充: 仅对"没有缓存"的板块速评逐个串行生成(避免并发打爆接口);
  // 已有缓存/本会话已跑过不再自动(手动按钮仍是全量重新生成)
  const autoRef = useRef(false);
  useEffect(() => {
    if (!autoFill || autoRef.current || groups.length === 0) return;
    autoRef.current = true;
    const ds = datasetOf(groups);
    const missing = groups
      .map((g) => sectorCacheKeyOf(ds, g.major))
      .filter((k) => {
        try {
  ...
  // 按页面顺序：主力期货 → 个股速评 → 仓单日报 → 各合约成交持仓 → 各板块
  const items = useMemo(
    () => [
      { key: `${FUTURE_KEY_PREFIX}_${dataset}`, label: "主力期货" },
      { key: `${STOCK_KEY_PREFIX}_${dataset}`, label: "个股速评" },
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-xl border border-border bg-card px-4 py-2.5">
      <p className="text-xs text-muted-foreground">
        一键生成主力期货/个股/仓单日报、成交持仓与全部板块速评；已生成过的可一键全部刷新
      </p>
      <Button
        variant="outline"
        size="sm"
