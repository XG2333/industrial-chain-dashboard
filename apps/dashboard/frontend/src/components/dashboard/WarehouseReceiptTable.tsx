...
export function WarehouseReceiptTable({ dataset, liveOnly = false }: { dataset: "lithium" | "tin" | "silicon"; liveOnly?: boolean }) {
  const [data, setData] = useState<WarehouseTableData | null>(null);
  // 数据源: liveOnly=true 时(当前页面要求)固定公开 API, 隐藏"本地Excel"切换;
  // liveOnly=false 保留 excel ⇄ live 切换(后续需要时恢复)
  const [sourceMode, setSourceMode] = useState<"excel" | "live">(liveOnly ? "live" : "excel");
  const mode = liveOnly ? "live" : sourceMode;
  const [refreshing, setRefreshing] = useState(false);
  useEffect(() => {
    if (liveOnly && sourceMode !== "live") setSourceMode("live");
  }, [liveOnly, sourceMode]);
  ...
  // 加载当前数据源(live=交易所公开API / excel=本地Excel); 手动刷新按钮复用
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!d) return;
      if (d.rows.length === 0 && mode === "live") {
        // 公开源暂无数据(如非交易日): 非 liveOnly 自动回退本地 Excel; liveOnly 保持空态提示
        if (!liveOnly) setSourceMode("excel");
        else setData(d);
        return;
      }
      setData(d);
    } catch {
      if (mode === "live" && !liveOnly) setSourceMode("excel");
    }
  }, [dataset, mode, liveOnly]);
  ...
  const refreshNow = () => {
    ...
    loadData()
      ...
      .then(() => quickAnalysisBus.run(`warehouse_analysis_v2_${dataset}`)); // 数据更新后联动仓单速评
  };

  // 无数据: 非 liveOnly 不渲染; liveOnly(仅公开API) 渲染标题行 + 空态提示
      ...
      <div id="warehouse-receipts" className="scroll-mt-28 border-b border-border">
        <div className="flex flex-wrap items-center justify-between gap-2 px-5 pt-3 text-lg">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-muted-foreground/70">单位：手 · 公开API(交易所/东财)</span>
          </div>
          <RefreshBtn onClick={refreshNow} loading={refreshing} title="立即刷新仓单数据" />
        </div>
        <p className="px-5 py-8 text-center text-xs text-muted-foreground">
          交易所暂无当日仓单数据(非交易日或数据源暂未更新), 可点击刷新重试
        </p>
      </div>
    );
  }
  ...
  const sortedOtherRows = [...regionRows]
    ...
    .concat(tailRows);
  // 全部仓库默认展开显示(不做收起)
  const shownWarehouses = sortedWarehouseRows;
  ...
  return (
    <div id="warehouse-receipts" className="scroll-mt-28 border-b border-border">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 px-5 pt-3 text-lg">
        <div className="flex items-center gap-2">
          <span className="font-bold text-foreground">仓单日报</span>
          <span className="text-[10px] text-muted-foreground/70" title={mode === "live" ? "公开API: 锂/硅=广期所官网仓单日报(仓库级), 锡=东财仓单(仅全国总量); 当日快照+近30交易日走势" : "本地Excel: 全量历史+地区汇总+走势"}>
            {mode === "live" ? "单位：手 · 公开API(交易所/东财)" : "单位：手"}
            {displayDates[0] ? ` · 数据截至 ${String(displayDates[0]).replace(/-/g, "/")}` : ""}
          </span>
        </div>
        {/* 右侧: 刷新 + 排序控件; 数据源切换仅非 liveOnly 时显示 */}
      ...
      <div className="px-4 pb-4">
        <WarehouseQuickAnalysis dataset={dataset} source={mode} />
      </div>
