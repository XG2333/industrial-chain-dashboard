...
const SUB_SECTORS = ["锡矿", "锡锭", "锡材", "锡下游", "锡其他"];
const DEFAULT_STOCKS: Record<string, string[]> = {};
// 细分板块产业链顺序（与后端 server.py STOCK_SUB_ORDER_TIN 保持一致）
const STOCK_SUB_ORDER = ["锡矿", "锡锭", "锡材", "锡下游"];
...
function loadStocks(tab: string, targets: Record<string, string[]> = DEFAULT_STOCKS): string[] {
  try { const r = localStorage.getItem("mt_stocks_v5_" + tab); return r ? JSON.parse(r) : (targets[tab] || []); }
  catch { return targets[tab] || []; }
}
function saveStocks(tab: string, codes: string[]) { localStorage.setItem("mt_stocks_v5_" + tab, JSON.stringify(codes)); }
function loadAnalysis(tab: string): AnalysisResult | null {
  try { const r = localStorage.getItem("mt_analysis_v4_" + tab); return r ? JSON.parse(r) : null; }
  catch { return null; }
}
function saveAnalysis(tab: string, data: AnalysisResult) { localStorage.setItem("mt_analysis_v4_" + tab, JSON.stringify(data)); }
function loadReports(tab: string): ReportItem[] {
  try { const r = localStorage.getItem("mt_reports_v4_" + tab); return r ? JSON.parse(r) : []; }
  catch { return []; }
}
function saveReports(tab: string, data: ReportItem[]) { localStorage.setItem("mt_reports_v4_" + tab, JSON.stringify(data)); }

const cc = (pct: number) => pct === 0 ? "text-muted-foreground" : pct > 0 ? "text-red-500" : "text-green-500";

export function Metals() {
  const [activeTab, setActiveTab] = useState(() => localStorage.getItem("mt_tab_v4") || "总览");
  ...
  useEffect(() => {
    fetch("/api/stock-targets")
      ...
      .then((data) => {
        const groups = data?.groups?.tin || {};
        setStockTargets(groups);
        setStockMeta(data?.stocks?.tin || {});
  ...
  const switchTab = (t: string) => {
    setActiveTab(t); localStorage.setItem("mt_tab_v4", t);
    ...
    setTimeout(() => {
      ...
      fetch("/api/battery/excel/meta", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ tab: t, sector: isSubSector ? t : undefined, dataset: isSubSector ? undefined : "tin", selected_only: isSubSector ? undefined : true }) })
  ...
  const runAnalysis = async () => {
    ...
    try {
      const r = await fetch("/api/battery/analyze", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ tab: activeTab, industry: "tin", stocks: stockCodes }) });
  ...
  // 产业新闻（近 7 天，按行业关键词过滤：锡）
  const fetchNews = useCallback(() => {
    fetch("/api/overview/news?industry=tin")
  ...
  // 自选股票：锡个股池（stockMeta 细分板块分组，板块顺序 STOCK_SUB_ORDER），
  ...
  const fetchExcel = async () => {
    ...
    try {
      ...
      const r = await fetch("/api/battery/excel/meta", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ tab: activeTab, sector: isSubSector ? activeTab : undefined, dataset: isSubSector ? undefined : "tin", selected_only: isSubSector ? undefined : true }) });
    ...
    <div className="rounded-xl border border-border bg-slate-200/60 p-3 space-y-3">
      ...
      {isOverviewTab && (
        <div className="overflow-hidden rounded-xl border bg-card">
          <WarehouseReceiptTable dataset="tin" />
        </div>
      )}
      {layoutMode === "1" ? (
        <>
  ...
  return (
    ...
    <PageContainer maxWidth="wide">
      ...
      <IndustryKpiBand dataset="tin" />
      {/* 主力期货总体分析(总览 KPI 下方, 参考速评) */}
      <div className="mt-3">
        <KpiFutureAnalysis dataset="tin" />
      </div>
